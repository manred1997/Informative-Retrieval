        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                inputs = {
                    ...
                    "label_type": batch[3].to(self.device),
                    "label_ministry": batch[4].to(self.device)
                }
    ...
    def evaluate(self, mode):
        ...
        for batch in tqdm(eval_loader):
            with torch.no_grad():
                ...
                logits_ministry = (outputs[2] > self.args.threshold).type(torch.float)
            ...
            out_type_label = None
            ministry_label = None
            out_ministry_label = None
            
            # prediction
            if type_label is None:
                type_label = logits_type.detach().cpu().numpy()
                out_type_label = inputs["label_type"].detach().cpu().numpy()
            else:
                type_label = np.append(type_label, logits_type.detach().cpu().numpy(), axis=0)

                out_type_label = np.append(
                    out_type_label, inputs["label_type"].detach().cpu().numpy(), axis=0
                )
        
            if ministry_label is None:
                ministry_label = logits_ministry.detach().cpu().numpy()
                out_ministry_label = inputs["label_ministry"].detach().cpu().numpy()
            else:
                ministry_label = np.append(ministry_label, logits_ministry.detach().cpu().numpy(), axis=0)

                out_ministry_label = np.append(
                    out_ministry_label, inputs["label_ministry"].detach().cpu().numpy(), axis=0
                )

        eval_loss = eval_loss / nb_eval_steps
        results = {"loss": eval_loss}

        type_label = np.argmax(type_label, axis=1)


        total_result = compute_metrics(type_label, out_type_label, ministry_label, out_ministry_label)
